...
# from ..db import SessionLocal, User, Chat, ChatMemory, ChatMessage
# from langchain_core.messages import BaseMessage

# class PgDurableStore:
#     # ------- identity -------
#     def ensure_user(self, *, user_id: int, first_name: Optional[str], last_name: Optional[str], username: Optional[str]) -> None:
#         with SessionLocal() as s:
#             row = s.execute(select(User).where(User.user_id == user_id)).scalar_one_or_none()
#             if row:
#                 s.execute(
#                     update(User)
#                     .where(User.user_id == user_id)
#                     .values(first_name=first_name, last_name=last_name, username=username)
#                 )
#             else:
#                 s.execute(
#                     insert(User).values(
...
#     def ensure_chat(self, *, chat_id: int, chat_type: str, title: Optional[str], user_id: Optional[int]) -> None:
#         with SessionLocal() as s:
#             row = s.execute(select(Chat).where(Chat.chat_id == chat_id)).scalar_one_or_none()
#             if row:
#                 s.execute(
#                     update(Chat)
#                     .where(Chat.chat_id == chat_id)
#                     .values(chat_type=chat_type, title=title, user_id=user_id)
#                 )
#             else:
#                 s.execute(
#                     insert(Chat).values(
...
#         with SessionLocal() as s:
#             row = s.execute(select(ChatMemory).where(ChatMemory.chat_id == chat_id)).scalar_one_or_none()
#             if row:
#                 s.execute(update(ChatMemory).where(ChatMemory.chat_id == chat_id).values(last_summary=text))
#             else:
#                 s.execute(insert(ChatMemory).values(chat_id=chat_id, last_summary=text))
#             s.commit()
...
#         with SessionLocal() as s:
#             s.execute(insert(ChatMessage).values(chat_id=chat_id, role=role, content=content))
#             s.commit()

#     def add_messages_bulk(self, *, chat_id: int, turns: Iterable[tuple[str, str]]) -> None:
#         # turns: iterable of (role, content)
#         with SessionLocal() as s:
#             s.execute(
#                 insert(ChatMessage),
#                 [{"chat_id": chat_id, "role": r, "content": c} for r, c in turns],
#             )
#             s.commit()
