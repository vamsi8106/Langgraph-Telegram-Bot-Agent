...
#     def __init__(self, inner: ChatOpenAI, model: str):
#         self._inner = inner
#         self._model = model
#         self._tracer = get_tracer("llm")

#     # langgraph / your code only calls .invoke(); proxy everything else via __getattr__
#     def invoke(self, messages):
#         status = "ok"
#         t0 = time.perf_counter()
#         with self._tracer.start_as_current_span("llm.invoke", attributes={"llm.model": self._model}):
#             try:
...
#         # delegate any other attribute/method to the real llm
#         return getattr(self._inner, name)

# def build_llm():
#     model = settings.openai_model
#     base = ChatOpenAI(model=model, timeout=getattr(settings, "openai_timeout_sec", 30))
